...
if(isset($_POST["search"]))
{
  ...
  $mineralToSearch = $_POST["mineralToSearch"];
  ...
  if($mineralToSearch!="")
  {
      $query = $query." NATURAL JOIN pym";
      $query = $query." INNER JOIN minerales ON pym.mid=minerales.mid";
  }
  $query = $query." WHERE proyectos.nombre LIKE '%".$nameToSearch."%' AND";
  if($mineralToSearch!="")
  {
      $query = $query." minerales.nombre LIKE '%".$mineralToSearch."%' AND";
  }
  ...
  $query = "SELECT * FROM proyectos NATURAL JOIN proyectosmineras";
  $query = $query." NATURAL JOIN pyl NATURAL JOIN lugares";
  $search_result = filterTable($query) -> fetchALL();
}
...
<div class= "h1">
  <center><h2 class="room-name">Mineras</h2></center>
</div>
...
<center>
  ...
  <div class="container">
    <!-- <div class="row col-md-10 col-md-offset-2 custyle"> -->
      ...
      <form align="center" action="mineras.php" method="post">
        <div class="row">
          ...
          <a class="card text-white bg-success mb-2 col-md-2">Mineral</a><br><br>
        </div>
        <div class="row">
          ...
          <select class="col-md-2" type="text" name="mineralToSearch" palceholder="Value To Search">
            <option value=""></option>
            <?php
            require("../../config/conexion_impar.php");
            $query = "SELECT nombre FROM minerales ORDER BY nombre";
            $result = $db -> prepare($query);
          	$result -> execute();
            $results = $result -> fetchALL();
            foreach ($results as $row){
        ...
        <div class="table-wrapper-scroll-y my-custom-scrollbar">
          <table class="table table-bordered table-striped mb-0">
            ...
            <tr>
              ...
              if($mineralToSearch!="")
              {
                  echo "<th class=\"text-center\">Mineral</th>";
              }
              ?>
            </tr>
            </thead>
